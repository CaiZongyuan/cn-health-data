...
def test_full_registry_is_signed_complete_and_installable(tmp_path: Path) -> None:
    ...
    for dataset_id, expected_release_id in EXPECTED_RELEASES.items():
        ...
        expected_count = 2 if dataset_id == "laboratory-cn" else 1
        assert len(dataset["releases"]) == expected_count
        for release in dataset["releases"]:
            ...
            assert manifest["runtime"]["minimumCliVersion"] in {"0.2.0", "0.4.0"}
            ...
            database_path = tmp_path / f"{dataset_id}-{release['storageKey']}.sqlite"
            with (
    ...
    assert total_compressed == 79_063_939
    assert total_uncompressed == 822_657_024
