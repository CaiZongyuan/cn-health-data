"""End-to-end Candidate build for WS/T 886."""
...
from cn_health_compiler.sources.nhc_lab.records import (
    NHCLaboratoryTestRecord,
    iter_nhc_laboratory_records,
)
from cn_health_compiler.sources.nhc_lab.sqlite import build_nhc_laboratory_sqlite
...
def _records(snapshot: SourceSnapshot, source_version: str) -> Iterable[NHCLaboratoryTestRecord]:
    return iter_nhc_laboratory_records(
        snapshot.path,
...
def _rules(path: Path) -> NHCLaboratoryValidationRules:
    return NHCLaboratoryValidationRules.model_validate(load_yaml_mapping(path)["validation"])


def _validation_payload(report: NHCLaboratoryValidationReport) -> dict[str, object]:
    return report.model_dump(mode="json")


_ADAPTER = FileCandidateAdapter(
    dataset_id="nhc-lab-tests",
    source_version_field="declared_version",
    table="laboratory_test",
    iter_records=_records,
    load_rules=_rules,
    build_sqlite=build_nhc_laboratory_sqlite,
    validation_payload=_validation_payload,
    excluded_diff_fields=("source_location", "source_row", "source_version", "source_sha256"),
)


def build_nhc_laboratory_candidate(
    repo_root: Path,
    source_path: Path,
    output_root: Path,
    *,
